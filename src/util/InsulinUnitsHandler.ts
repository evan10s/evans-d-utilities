...
export function convertInsulinUnitsToMilliunits(input: string | number, suppressWarnings: boolean = false): number {
	let parsed;
...
export function convertInsulinMilliunitsToUnits(input: number, suppressWarnings: boolean = false): number {
	if (!suppressWarnings) {
		if (input < 0) {
